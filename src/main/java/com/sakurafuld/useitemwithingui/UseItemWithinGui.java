...
@Mod(UseItemWithinGui.ID)
public class UseItemWithinGui {
    public static final String ID = "useitemwithingui";
    public static final Logger LOG = LoggerFactory.getLogger(ID);

    @SuppressWarnings("removal")
    public UseItemWithinGui() {
        this(FMLJavaModLoadingContext.get());
    }

    public UseItemWithinGui(FMLJavaModLoadingContext context) {
        DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> {
            MinecraftForge.EVENT_BUS.addListener(this::useItemWithinGui);
            MinecraftForge.EVENT_BUS.addListener(this::initUIWGHint);
            MinecraftForge.EVENT_BUS.addListener(this::closingUIWGHint);
            MinecraftForge.EVENT_BUS.addListener(this::checkUIWGHint);
            MinecraftForge.EVENT_BUS.addListener(this::renderUIWGHint);

            context.registerConfig(ModConfig.Type.CLIENT, UIWGConfig.SPEC);
        });
    }
        ...
        if (event.getButton() == InputConstants.MOUSE_BUTTON_RIGHT && Screen.hasControlDown() && player != null && event.getScreen() instanceof AbstractContainerScreen<?> screen) {
            if (screen instanceof CreativeModeInventoryScreen creative) {
                ...
                if (selectedIndex == hovered.index) {
                    ...
                    if (ItemHandlerHelper.canItemStacksStack(hovered.getItem(), selected.getItem())) {
                        this.useItem(player);
    ...
    private boolean handleCreative(LocalPlayer player, CreativeModeInventoryScreen screen) {
        ...
        if (!alt) {
            ...
            boolean different = !ItemHandlerHelper.canItemStacksStack(before, after);
            if (different) {
                if (infinite) {
                    if (!last.isEmpty()) {
                        player.drop(true);
